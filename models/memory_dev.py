...
def gumbel_softmax(att_weight, topk=1):
    y = F.softmax(att_weight, dim=-1)    # shape (batch, num_patches, num_slots)

    thres, _ = torch.topk(y, topk, dim=-1, sorted=True)
    thres = thres[..., [-1]]    # shape (batch, num_patches, 1)

    y_hard = y.detach().clone()
    y_hard = hard_shrink_relu(y_hard, lambd=thres)  # [N,M]

    # normalize
    y_hard = F.normalize(y_hard, p=1, dim=-1)  # 1 / topk
...
class MemoryQueue(nn.Module):
    def __init__(self, num_memory, num_slots, slot_dim, shrink_thres=0.0025):
        super(MemoryQueue, self).__init__()
        self.num_memory = num_memory
        self.num_slots = num_slots
        self.slot_dim = slot_dim

        memMatrix = torch.zeros(num_memory, num_slots, slot_dim)
    ...
    def reset_parameters(self):
        stdv = 1. / math.sqrt(self.memMatrix.size(2))
    ...
    def update(self):
        ...
        # values: B, C
        values = self.values    # shape (batch, num_memory, slot_dim)

        # only support single gpu at this stage
        values = values.permute(1, 0, 2)
        num_memory, batch_size, slot_dim = values.shape

        if self.ptr + batch_size < self.num_slots:
            self.memMatrix[:, self.ptr:self.ptr+batch_size] = values
        else:
            self.memMatrix[:, self.ptr:] = values[:, :self.num_slots - self.ptr]
            offset = (self.ptr + batch_size) % self.num_slots
            self.memMatrix[:, :offset] = values[:, self.num_slots - self.ptr:]
        
        self.ptr = (self.ptr + batch_size) % self.num_slots
        self.values = None
        del self.values
    ...
    def forward(self, x):
        """
        Args:
            x: shape (batch_size, num_patches, slot_dim), where num_patches == num_memory
        Returns:
            query output retrieved from memory, with the same size as x.
        """
        # dot product
        # att_weight: shape (batch, num_memory, 1, num_slots)
        att_weight = torch.matmul(x.unsqueeze(2), self.memMatrix.unsqueeze(0).permute(0, 1, 3, 2))
        
        if self.shrink_thres > 0 and type(self.shrink_thres) is int:
            att_weight = gumbel_softmax(att_weight, topk=self.shrink_thres)
        elif self.shrink_thres > 0 and type(self.shrink_thres) is float:
            att_weight = hard_shrink_relu(att_weight, lambd=self.shrink_thres)  # [N,M]
            att_weight = F.normalize(att_weight, p=1, dim=1)
    
        out = torch.matmul(att_weight, self.memMatrix).squeeze(2)
        return out
